...
class ConsultarPixRecebidos
{
    private $txIdQuery = "&txId=";
    private $cpfQuery = "&cpf=";
    private $cnpjQuery = "&cnpj=";
    private $txIdPresenteQuery = "&txIdPresente=";
    private $devolucaoPresenteQuery = "&devolucaoPresente=";
    private $paginaAtualQuery = "&paginacao.paginaAtual=";
    private $itensPorPaginaQuery = "&paginacao.itensPorPagina=";
    private $msg = "Chamada Iniciada - Consulta PIX recebidos";
    private $errorCallApiMessage = "Erro ao Consultar Pix Recebidos. ";
                                                ...
                                                FiltrosPixRecebidos $filtros = null, Paginacao $paginacao = null)
    {
        ...
        while ($continue) {
            ...
            $pix = array_merge((array)$pix, (array)$body->pix);
     ...
     * @throws ServerException
     */
    private function call(string    $dataInicio, string $dataFim, FiltrosPixRecebidos $filtros = null,
                          Paginacao $paginacao = null): object
    {
        ...
        $path = ConfigUtils::getPath(API_TYPE_PIX, API_ENDPOINT_PATH_GET_PIX_RECEBIDOS);
    ...
    private function buildQuery(string    &$URL, FiltrosPixRecebidos $filtros = null,
                                Paginacao $paginacao = null): void
    {
        if ($filtros !== null) {
            $this->buildFiltersQuery($URL, $filtros);
        }
    ...
    private function buildFiltersQuery(string &$URL, FiltrosPixRecebidos $filtros): void
    {
        ...
        if ($filtros->getDevolucaoPresente() !== null) {
            $URL .= $this->devolucaoPresenteQuery . var_export($filtros->getDevolucaoPresente(), true);
        }

        if ($filtros->getTxIdPresente() !== null) {
            $URL .= $this->txIdPresenteQuery . var_export($filtros->getTxIdPresente(), true);
        }

        if ($filtros->getTxId() !== null) {
            $URL .= $this->txIdQuery . $filtros->getTxId();
        }
    }

    final private function buildMsg(string              $dataInicio, string $dataFim, bool $buscaPaginada,
                                    FiltrosPixRecebidos $filtros = null, Paginacao $paginacao = null): string
    {
        ...
        if ($filtros !== null) {
            $mensagem .= LINE_BREAK .
                TX_ID_PRESENTE . var_export($filtros->getTxIdPresente(), true) . LINE_BREAK .
                CPF . $filtros->getCpf() . LINE_BREAK .
                CNPJ . $filtros->getCnpj() . LINE_BREAK .
                TXID . $filtros->getTxId() . LINE_BREAK .
                DEVOLUCAO_PRESENTE . var_export($filtros->getDevolucaoPresente(), true);
        }
